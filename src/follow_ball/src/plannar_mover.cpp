  ...
  class PlannarMover : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&PlannarMover::OnUpdate, this)); //bind genera una funcion parcial
      
      this->old_secs =ros::Time::now().toSec();
      
      // Create a topic name
      std::string plannar_pos_topicName = "/cmd_vel"; //crea un objeto de la clase string (caracteres)
